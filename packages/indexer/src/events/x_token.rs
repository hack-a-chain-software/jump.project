    ...
    async fn sql_query(&self, conn: &mut PgPooledConnection) {
        match &self {
            Self::FtMint([FtMintLog {
                ...
                let base_token: u128 = memo_json["base_token_treasury_after_deposit"].as_str().unwrap().parse().unwrap();
                let x_token: u128 = memo_json["x_token_supply_after_deposit"].as_str().unwrap().parse().unwrap();
                let event_timestamp: u64 = memo_json["timestamp"].as_str().unwrap().parse().unwrap();
            ...
            Self::ProfitDeposit([ProfitDepositLog { 
                ...
                timestamp }]) => {
                    ...
                    ",
                        &[
                            &U64toUTC(*timestamp),
                            &Decimal::from_u128(base_token_treasury_after_deposit.0).unwrap(),
                            &Decimal::from_u128(x_token_supply_after_deposit.0).unwrap(),
                        ],
                    )
                    .await
                    .unwrap();
