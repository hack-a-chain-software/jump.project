...
mod tests {
  ...
  async fn test_normal_flow() -> anyhow::Result<()> {
    ...
    ft_transfer(&worker, &minter, &locked_token, &user, send_amount).await?;

    ft_transfer(&worker, &minter, &locked_token, &user2, send_amount).await?;

    ft_transfer(&worker, &minter, &locked_token, &user3, send_amount).await?;

    let vesting_1_1 = &view_vesting_paginated(&worker, &locked_token, &user).await?[0];
    let vesting_2_1 = &view_vesting_paginated(&worker, &locked_token, &user2).await?[0];
    let vesting_3_1 = &view_vesting_paginated(&worker, &locked_token, &user3).await?[0];
    ...
    let cost = (vesting_1_1["locked_value"]
      ...
      * 500)
      / 10000;
    let initial_x_token: u128 = ft_balance_of(&worker, &ft_token, x_token.as_account())
      .await?
      .parse()
      .unwrap();
    let initial_user: u128 = ft_balance_of(&worker, &ft_token, &user)
      .await?
      .parse()
      .unwrap();
    ...
    let final_x_token: u128 = ft_balance_of(&worker, &ft_token, x_token.as_account())
      .await?
      .parse()
      .unwrap();
    let final_user: u128 = ft_balance_of(&worker, &ft_token, &user)
      .await?
      .parse()
      .unwrap();
    ...
    let initial_user: u128 = ft_balance_of(&worker, &ft_token, &user)
      ...
      .parse()
      .unwrap();
    let initial_user2: u128 = ft_balance_of(&worker, &ft_token, &user2)
      .await?
      .parse()
      .unwrap();
    let initial_user3: u128 = ft_balance_of(&worker, &ft_token, &user3)
      .await?
      .parse()
      .unwrap();

    withdraw_locked_tokens(&worker, &user, &locked_token, "0".to_string()).await?;
    withdraw_locked_tokens(&worker, &user2, &locked_token, "0".to_string()).await?;
    withdraw_locked_tokens(&worker, &user3, &locked_token, "0".to_string()).await?;

    let final_user: u128 = ft_balance_of(&worker, &ft_token, &user)
      .await?
      .parse()
      .unwrap();
    let final_user2: u128 = ft_balance_of(&worker, &ft_token, &user2)
      .await?
      .parse()
      .unwrap();
    let final_user3: u128 = ft_balance_of(&worker, &ft_token, &user3)
      .await?
      .parse()
      .unwrap();

    let vesting_1_2 = &view_vesting_paginated(&worker, &locked_token, &user).await?[0];
    let vesting_2_2 = &view_vesting_paginated(&worker, &locked_token, &user2).await?[0];
    let vesting_3_2 = &view_vesting_paginated(&worker, &locked_token, &user3).await?[0];
