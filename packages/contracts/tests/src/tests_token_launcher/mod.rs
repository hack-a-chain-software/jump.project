...
mod tests {
  ...
  async fn test_normal_flow() -> anyhow::Result<()> {
    ...
    let _user2 = create_user_account(&root, &worker, "user2").await;
    let _user3 = create_user_account(&root, &worker, "user3").await;
    ...
    let factory = deploy_contract(&root, &worker, &FACTORY, &token_launcher_wasm).await;
    //initialize the factory contract
    initialize_factory_contract(&worker, &factory, &owner).await;

  
    let wasm = get_wasm("token_contract.wasm").unwrap();
    let hash: String = owner.call(&worker, factory.id(), "store").args(wasm)
    .deposit(DEPOSIT)
    .gas(GAS_LIMIT)
    .transact()
    .await?.json()?;
    ...
    register_contract(&worker, &owner, &factory, CONTRACT_NAME.to_string(), hash.clone(), 5, INIT.to_string(), "new".to_string()).await;

    deploy_new_contract(&worker, &owner, &factory,CONTRACT_NAME.to_string(), PREFIX.to_string(), params).await;
    

    let deployed_name: workspaces::AccountId = format!("{}.{}", PREFIX , factory.id()).parse().unwrap();

    let balance: String = user.call(
      &worker,
      &deployed_name,
      "ft_balance_of")
      .args_json(json!({
    ...
    let owner_balance_before_callback = owner.view_account(&worker).await?.balance;

    println!("{}{}","balance before callback: ", owner_balance_before_callback);

    deploy_new_contract(&worker, &owner, &factory,CONTRACT_NAME.to_string(), "token_do_jasso".to_string(), wrong_params).
    await;

    let owner_balance_after_callback = owner.view_account(&worker).await?.balance;

    println!("{}{}","balance after callback: ", owner_balance_after_callback);
    // assuming gas cost < 1 
    assert!(owner_balance_after_callback >= (owner_balance_before_callback - 1_000_000_000_000_000_000_000_000));
