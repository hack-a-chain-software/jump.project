...
mod tests {
    ...
    use crate::tests::{
        ...
        SIGNER_ACCOUNT, OWNER_ACCOUNT, TOKEN_ACCOUNT
    };
    use super::*;
    ...
    fn test_register_storage_happy() {
        ...
        let mut contract = StakingFT::initialize_staking(OWNER_ACCOUNT.to_string(), TOKEN_ACCOUNT.to_string(), U128(1), U128(100));

        contract.register_storage(test_account.clone());

        let user_data = contract.user_map.get(&test_account).expect("User not registered");
